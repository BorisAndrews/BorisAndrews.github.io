...
### AUG.2026 ([arXiv](https://doi.org/10.48550/arXiv.2511.23266)) <code>&#124;</code> In review ({% include journals/camwa.md %})
...
<div class="reveal-box" onclick="var details = this.querySelector('.details'); details.style.display = (details.style.display === 'block') ? 'none' : 'block';">
    <b>FULL ABSTRACT</b>
    <div class="details">
        Ordinary and partial differential equations describing thermodynamically isolated systems typically possess conserved quantities (like mass, momentum, and energy) and dissipated quantities (like entropy).
...
In this manuscript, {% include collaborators/patrick/short.md %} and I employ our [auxiliary variable framework](/publications/sp-integrators-a/) to construct an integrator for **general ODE systems** with **multiple invariants** that conserves **every prescribed invariant**.

The trick is to first rewrite the system so its right-hand side is generated by an **alternating form**, contracted against the gradients of the prescribed invariants \(N_1, \dots, N_P\).
These gradients may be substituted for **discrete gradients** / **auxiliary variables**, guaranteeing the conservation is preserved to the discrete level.
This is inspired by the [**Nambu bracket**](https://en.wikipedia.org/wiki/Nambu_mechanics), an antisymmetric bracket which generalises the Poisson bracket to *several* invariants at once:
\(\dot{\mathbf{x}} = \{\mathbf{x}, N_1, \dots, N_P\}\).

Here's the integrator on the same Kepler problem: 
